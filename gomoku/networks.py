...
class _Stack(tf.Module):
  ...
  def __init__(self, num_ch, num_blocks):
    ...
    self._conv = tf.keras.layers.Conv2D(num_ch, 3, strides=1, padding='same', kernel_initializer='orthogonal')
    self._max_pool = tf.keras.layers.MaxPool2D(
        pool_size=3, padding='same', strides=2)

    self._res_convs0 = [
        tf.keras.layers.Conv2D(
            num_ch, 3, strides=1, padding='same', name='res_%d/conv2d_0' % i, kernel_initializer='orthogonal')
        for i in range(num_blocks)
    ]
    self._res_convs1 = [
        tf.keras.layers.Conv2D(
            num_ch, 3, strides=1, padding='same', name='res_%d/conv2d_1' % i, kernel_initializer='orthogonal')
        for i in range(num_blocks)
    ]
...
class ImpalaDeep(tf.Module):
  ...
  def __init__(self, num_actions):
    ...
    self._core = tf.keras.layers.LSTMCell(512, kernel_initializer='orthogonal')
    ...
    self._conv_to_linear = tf.keras.layers.Dense(512, kernel_initializer='orthogonal')
