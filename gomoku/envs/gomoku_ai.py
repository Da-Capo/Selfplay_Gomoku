...
def strategy(state):
    ...
    def getstring(point):
        ...
        # vertical
        getLine1 = ''
        for k in range(max(x - 4, 0), min(x + 5, board_size)):
            getLine1 += str(int(table[k, y]))
        if x-4<0: getLine1 = '*'+getLine1
        if x+4>14: getLine1 = getLine1+'*'
        # horizonal
        getLine2 = ''
        for k in range(max(y - 4, 0), min(y + 5, board_size)):
        ...
        bx = max(0, x - 4)
        by = max(0, y - 4)
        ux = min(board_size-1, x + 4)
        uy = min(board_size-1, y + 4)
    ...
    if len(board[0]) == 0 and len(board[1]) == 0:
        ...
        xmax = min(max(sumBoard, key=lambda x: x[0])[0] + 2, board_size)
        ymax = min(max(sumBoard, key=lambda x: x[1])[1] + 2, board_size)
