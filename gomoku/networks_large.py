...
kernel_initializer = 'orthogonal' #orthogonal glorot_uniform
class _Stack(tf.Module):
  """Stack of pooling and convolutional blocks with residual connections."""

  def __init__(self, num_ch, num_blocks):
    super(_Stack, self).__init__(name='stack')
    self._conv = tf.keras.layers.Conv2D(num_ch, 3, strides=1, padding='same', kernel_initializer=kernel_initializer)

    self._layernorm = tf.keras.layers.LayerNormalization()

    self._res_convs0 = [
        tf.keras.layers.Conv2D(
            num_ch, 3, strides=1, padding='same', name='res_%d/conv2d_0' % i, kernel_initializer=kernel_initializer)
        for i in range(num_blocks)
    ]
    self._res_convs1 = [
        tf.keras.layers.Conv2D(
            num_ch, 3, strides=1, padding='same', name='res_%d/conv2d_1' % i, kernel_initializer=kernel_initializer)
        for i in range(num_blocks)
    ]
  ...
  def __call__(self, conv_out):
    ...
    for (res_conv0, res_conv1) in zip(self._res_convs0, self._res_convs1):
      ...
      conv_out = tf.nn.leaky_relu(conv_out)
      conv_out = res_conv0(conv_out)
      conv_out = tf.nn.leaky_relu(conv_out)
      conv_out = res_conv1(conv_out)
      conv_out += block_input
      conv_out = self._layernorm(conv_out)  
...
class ImpalaDeep(tf.Module):
  ...
  def __init__(self, num_actions):
    ...
    self._core = tf.keras.layers.LSTMCell(1024, kernel_initializer=kernel_initializer)

    # self._conv = tf.keras.layers.Conv2D(128, 3, strides=1, padding='same', kernel_initializer=kernel_initializer)
    # self._layernorm = tf.keras.layers.LayerNormalization()

    # Parameters and layers for _torso.
    self._stacks = [
        _Stack(num_ch, num_blocks)
        for num_ch, num_blocks in [(32, 1), (64, 1), (64, 1), (64, 1),
                                    (64, 1), (64, 1), (64, 1), (64, 1),
                                    (64, 1), (64, 1), (64, 1), (64, 1)
                                    ]
    ]
    self._conv_to_linear = tf.keras.layers.Dense(512, kernel_initializer=kernel_initializer)

    # Layers for _head.
    self._policy_logits = tf.keras.layers.Dense(
        self._num_actions, name='policy_logits', kernel_initializer=tf.zeros_initializer())
  ...
  def _head(self, core_output):
    ...
    action_mask = tf.cast(tf.equal(tf.reshape(self.valid, tf.shape(policy_logits)),0), tf.float32)
  ...
  def _unroll(self, prev_actions, env_outputs, core_state):
    ...
    for input_, d in zip(tf.unstack(torso_outputs), tf.unstack(done)):
      ...
      core_state = tf.nest.map_structure(
          ...
          initial_core_state,
          core_state)
      # tf.print('init core_state', core_state)
      core_output, core_state = self._core(input_, core_state)
      core_output_list.append(core_output)
    core_outputs = tf.stack(core_output_list)
    # tf.print('core outputs', core_outputs)
