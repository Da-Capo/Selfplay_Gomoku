...
# Training.
flags.DEFINE_integer('max_to_keep', 1, 'checkpoint num max_to_keep.')
...
# Logging
flags.DEFINE_integer('log_batch_frequency', 200, 'We average that many batches '
                     'before logging batch statistics like entropy.')
flags.DEFINE_integer('log_episode_frequency', 200, 'We average that many episodes'
...
def learner_loop(create_env_fn, create_agent_fn, create_optimizer_fn):
  ...
  manager = tf.train.CheckpointManager(
      ckpt, FLAGS.logdir, max_to_keep=FLAGS.max_to_keep, keep_checkpoint_every_n_hours=6)
...
#########################################################
  def add_batch_size_eval(ts):
    return tf.TensorSpec([1] + list(ts.shape),
                         ts.dtype, ts.name)
  ...
  inference_specs_eval = tf.nest.map_structure(add_batch_size_eval, inference_specs_eval)
  ...
  def inference_eval(actor_ids, run_ids, env_outputs, raw_rewards):
    ...
    # Distribute the inference calls among the inference cores.
    branch_index = inference_iteration_eval.assign_add(1) % len(inference_devices)
    agent_outputs, curr_agent_states = tf.switch_case(branch_index, {
        i: make_inference_fn(inference_device)
        for i, inference_device in enumerate(inference_devices)
    })
